...
<script>
    $(document).ready(function () {
        var beritaId = window.location.pathname.split('/').pop();

        $.ajax({
            url: '/api/admin/berita/edit/' + beritaId,
            method: 'GET',
            success: function(data) {
                // Isi nilai input form dengan data yang diambil dari API
                $('#judul_berita').val(data.berita.judul_berita);
                $('#tgl_berita').val(data.berita.tgl_berita);
                $('#isi_berita').val(data.berita.isi_berita);
                // Tampilkan preview gambar jika ada
                if (data.berita.gambar) {
                    var previewHtml = '<img src="/images/berita/' + data.berita.gambar + '" class="img-fluid">';
